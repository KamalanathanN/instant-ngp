...
def render_video(resolution, numframes, scene, name, spp, fps, exposure=0, video_outpath="."):
	testbed = ngp.Testbed(ngp.TestbedMode.Nerf)
	# testbed.load_snapshot("data/toy/base.msgpack")
	# testbed.load_camera_path("data/toy/base_cam.json")
	testbed.load_snapshot(os.path.join(scene, "1000.msgpack"))
	...
	video_file_path = os.path.join(video_outpath, f"{name}_test.mp4")
	os.system(f"ffmpeg -i temp/%04d.jpg -vf \"fps={fps}\" -c:v libx264 -pix_fmt yuv420p {video_file_path}")
	shutil.rmtree('temp')
...
def parse_args():
	...
	parser.add_argument("--video_outpath", type=str, default=".", help="Output path for the rendered video")
...
if __name__ == "__main__":
	...
	render_video([args.width, args.height], args.n_seconds*args.fps, args.scene, args.render_name, spp=8, fps=args.fps, video_outpath=args.video_outpath)